...
public class BinarySearchTree<E> extends AbstractSet<E>
    ...
    private boolean isInLevelOrderTraverse, isInInOrderTraverse;
    private final int VISITTIME = 500;
    ...
    public void resetElementVisited() {
        if (rootNode != null) {
            Queue<BinaryTreeNode> q = new Queue<BinaryTreeNode>();
            q.enqueue(rootNode);
            while (!q.isEmpty()) {
                BinaryTreeNode temp = q.getFront();
                temp.visit = false;
            ...
            isInInOrderTraverse = false;
        }
    }

    public void leftRotate(E data) {
        BinaryTreeNode nodeToBeRoatated = search(data);
        if (nodeToBeRoatated != null) {
            BinaryTreeNode granparentNode = nodeToBeRoatated;

            setNode(nodeToBeRoatated.leftChild,nodeToBeRoatated);
        }
    }

    public BinaryTreeNode search(E data) {
        BinaryTreeNode focusNode = rootNode;

        while (compare(focusNode.element, data) != 0) {
            if ((compare(data, focusNode.element) < 0)) {
                focusNode = focusNode.leftChild;
            } else {
                focusNode = focusNode.rightChild;
            }
            if (focusNode == null) {
                System.out.println("Searched for " + data + " and no results found.");
                return null;
            }
        }
    ...
    public void setNode(BinaryTreeNode nodeToBeReplaced,BinaryTreeNode newNode) {
        BinaryTreeNode focusNode = rootNode;

        while (compare(focusNode.element, nodeToBeReplaced.element) != 0) {
            if ((compare(nodeToBeReplaced.element, focusNode.element) < 0)) {
                focusNode = focusNode.leftChild;
            } else {
                focusNode = focusNode.rightChild;
            }
            if (focusNode == null) {
                System.out.println("Searched for " + nodeToBeReplaced.element + " and no results found.");
                return;
            }
        }
        System.out.println("Found " + nodeToBeReplaced.element + ".");
        focusNode=nodeToBeReplaced;
    }

    public String LevelOrderTraverse(JPanel mainPanel) {
        if (rootNode != null && !isInLevelOrderTraverse && !isInInOrderTraverse) {
            isInLevelOrderTraverse = true;// Boolean to keep the user from spaming this method.
            String str = "[";
            Queue<BinaryTreeNode> q = new Queue<BinaryTreeNode>();
            q.enqueue(rootNode);
            while (!q.isEmpty()) {
            ...
            resetElementVisited();
            mainPanel.repaint();
            isInLevelOrderTraverse = false;
            return str;
        }
        return "No root node.";
    }

    public void inOrderTraverse(BinaryTreeNode rootNode, JPanel mainPanel) {
        if (rootNode != null && !isInInOrderTraverse && !isInLevelOrderTraverse) {
            inOrderTraverse(rootNode.leftChild, mainPanel);
            rootNode.visit = true;
            mainPanel.repaint();
            try {
                Thread.sleep(VISITTIME);
            ...
            inOrderTraverse(rootNode.rightChild, mainPanel);
        }
    }
